...
t[#t+1] = Def.ActorFrame {
    LoadActor("CharactSingle") .. {
	    InitCommand=cmd(diffusealpha,0;x,-50;y,480;zoomx,1.2;rotationz,180;rotationy,180;fadetop,0.5;croptop,0.5);		
		OffCommand=cmd(smooth,0.3;zoom,0;diffusealpha,0;);
		OnCommand=cmd(sleep,0.6;linear,0.1;diffusealpha,0.1;zoomy,1.8;linear,0.1;zoomy,2;zoomx,1.8;linear,0.1;zoomx,2);
	};
};	
----------## DANCE PAD FRAME ##----------
t[#t+1] = Def.ActorFrame {
    LoadActor("PadSingle") .. {
	    InitCommand=cmd(diffusealpha,0;x,0;y,180);
    ...
    LoadActor("PadSingle") .. {
		...
		GainFocusCommand=cmd(zoom,1.7;playcommand,"Animate");
    ...
    LoadActor("PadSingle") .. {
	    ...
	    InitCommand=cmd(diffusealpha,0;x,-30;y,180);
        OnCommand=cmd(blend,'BlendMode_Add';sleep,0.5;linear,0.1;diffusealpha,0;zoom,1.8;linear,0.1;zoom,1.3;linear,0.2;zoom,1.7);
		GainFocusCommand=cmd(zoom,1.7;playcommand,"Animate");
		AnimateCommand=cmd(linear,0.5;diffusealpha,0.5;linear,0.5;diffusealpha,0;queuecommand,"Animate");
...
-- IF ONLY 1 PLAYER ENABLE -----
	LoadActor("SinglepanelExpl") .. {
        Condition=GAMESTATE:GetNumPlayersEnabled()==1;
	    InitCommand=cmd(diffusealpha,0;x,230;y,-45;zoom,2);
		OnCommand=cmd(zoomy,0;sleep,1.2;linear,0.2;diffusealpha,1.0;zoomy,2);
		LoseFocusCommand=cmd(stoptweening;smooth,0.2;zoomy,0);
		GainFocusCommand=cmd(stoptweening;sleep,1;linear,0.2;diffusealpha,1.0;zoomy,2);
		OffCommand=cmd(smooth,0.3;zoom,0;diffusealpha,0;);
	};
	-- IF 2 PLAYERS ENABLE -----
    LoadActor("SinglepanelExpl") .. {
        Condition=GAMESTATE:GetNumPlayersEnabled()==2;
		InitCommand=cmd(diffusealpha,0;x,150;y,-60;zoom,2);
...
t[#t+1] = Def.ActorFrame {
    LoadActor("CharactSingle") .. {
	    InitCommand=cmd(diffusealpha,0;x,-50;y,-110;zoomx,1.2);		
		OffCommand=cmd(smooth,0.3;zoom,0;diffusealpha,0;);
